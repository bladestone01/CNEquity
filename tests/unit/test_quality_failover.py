...
from cnequity.quality import failover as fo
...
def test_write_backup_snapshot_handles_no_path_returned(tmp_path, monkeypatch):
    cfg = _cfg(tmp_path)
    monkeypatch.setattr("cnequity.quality.failover.SnapshotStore.write", lambda self, *a, **k: None)
...
def test_snapshot_daily_bars_clist_fetches_when_df_not_provided(tmp_path, monkeypatch):
    ...
    calls: list = []
    monkeypatch.setattr(
        "cnequity.quality.failover.fetch_daily_bars_clist",
...
def test_snapshot_corporate_actions_tdx_backup_noop_when_empty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cnequity.adapters.tdx_protocol.client.quotes_client_factory",
        lambda config: lambda: None,
    )
    monkeypatch.setattr(
        "cnequity.adapters.tdx_protocol.corporate_actions.fetch_corporate_actions_tdx",
