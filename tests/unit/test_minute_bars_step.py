...
def test_step_is_a_no_op_when_disabled(cfg):
    result = step_minute_bars(cfg, date(2026, 7, 31), "run-1", {})
    assert result["rows_written"] == 0
    assert "disabled" in result["note"]


def test_step_rejects_unknown_frequency(cfg):
    cfg.minute_bars_enabled = True
    cfg.minute_bars_frequency = "2m"
    with pytest.raises(ValueError, match="not supported"):
        step_minute_bars(cfg, date(2026, 7, 31), "run-1", {})
