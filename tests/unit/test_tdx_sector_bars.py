...
from cnequity.adapters.tdx_protocol.sector_bars import (
...
def test_fetch_sector_index_bars_batch(tmp_path):
    ...
    with patch(
        "cnequity.adapters.tdx_protocol.sector_bars.fetch_sector_index_bars",
...
def test_fetch_sector_index_bars_pagination(tmp_path):
    ...
    with (
        patch(
            "cnequity.adapters.tdx_protocol.sector_bars.fetch_bars_paginated",
            return_value=fake,
        ),
        patch(
            "cnequity.adapters.tdx_protocol.sector_bars._quotes_client",
            return_value=MagicMock(),
        ),
        patch(
            "cnequity.adapters.tdx_protocol.sector_bars.close_quotes_client",
        ),
    ):
