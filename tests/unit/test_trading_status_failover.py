...
# --- run guards (trading-status-window-guard-and-diagnostics) ---


def _write_curated_trading_status(config, trade_date, symbols):
    root = Path(config.curated_root) / "trading_status" / f"trade_date={trade_date.isoformat()}"
    root.mkdir(parents=True, exist_ok=True)
    df = pl.DataFrame(
        {
            "symbol": symbols,
            "trade_date": pl.Series([trade_date] * len(symbols), dtype=pl.Date),
            "is_trading": [True] * len(symbols),
            "status": ["normal"] * len(symbols),
        }
    )
    df.write_parquet(root / "part.parquet")
    return root


def test_covered_day_short_circuits(monkeypatch, config):
    d = date(2026, 8, 18)
    syms = ["000001.SZ", "600519.SH"]
    _write_curated_trading_status(config, d, syms)
    ...
    monkeypatch.setattr(steps.reference, "fetch_trading_status", _boom)
    result = steps.reference.step_trading_status(config, d, "r", {"symbols": syms})
    assert result["rows_written"] == 0
    assert touched["called"] is False
    findings = result["context_updates"]["audit_findings"]
    assert findings[0]["check"] == "already_completed"
    assert "已补齐" in findings[0]["detail"]


def test_partial_coverage_does_not_short_circuit(monkeypatch, config):
    d = date(2026, 8, 18)
    syms = ["000001.SZ", "600519.SH", "300750.SZ"]
    _write_curated_trading_status(config, d, syms[:2])  # partial
    frame = _ts_frame(syms, d)
    ...
    monkeypatch.setattr(steps.reference, "write_simple", _write)
    result = steps.reference.step_trading_status(config, d, "r", {"symbols": syms})
    assert result["rows_written"] == 3  # fetch proceeded despite partial coverage


def test_before_cutoff_skips_with_advisory(monkeypatch, config):
    d = date(2026, 8, 20)  # current trading day

    monkeypatch.setattr(
        steps.reference,
        "shanghai_now",
        lambda now=None: datetime(2026, 8, 20, 9, 15),
    )
    touched = {"called": False}
    ...
    monkeypatch.setattr(steps.reference, "fetch_trading_status", _boom)
    result = steps.reference.step_trading_status(config, d, "r", {"symbols": ["000001.SZ"]})
    assert result["status"] == "warning"
    assert touched["called"] is False
    findings = result["context_updates"]["audit_findings"]
    assert findings[0]["check"] == "before_cutoff"
    assert "16:00" in findings[0]["detail"]


def test_after_cutoff_proceeds(monkeypatch, config):
    d = date(2026, 8, 20)

    monkeypatch.setattr(
        steps.reference, "shanghai_now", lambda now=None: datetime(2026, 8, 20, 16, 0)
    )
    syms = ["000001.SZ"]
    ...
    monkeypatch.setattr(steps.reference, "fetch_incremental_daily", _incremental)
    monkeypatch.setattr(steps.reference, "fetch_trading_status", lambda *a, **k: _ts_frame(syms, d))
    captured = {}
...
def test_historical_day_morning_is_eligible(monkeypatch, config):
    d = date(2026, 8, 19)

    # now is the next trading day morning; D < today → eligible
    monkeypatch.setattr(
        steps.reference, "shanghai_now", lambda now=None: datetime(2026, 8, 20, 9, 15)
    )
    syms = ["000001.SZ"]
...
def test_backup_decline_reason_surfaces(monkeypatch, config):
    d = date(2026, 8, 18)
    syms = ["000001.SZ"]
    ...
    monkeypatch.setattr(steps.reference, "fetch_trading_status", _primary_fail)
    monkeypatch.setattr(
        steps.reference,
        "fetch_trading_status_backup",
        lambda cfg, symbols, day: (
            None,
            {"failover_used": False, "reason": "baostock has no data for 2026-08-18 yet"},
        ),
    )
    import pytest

    with pytest.raises(RuntimeError, match="backup declined: baostock has no data"):
        steps.reference.step_trading_status(config, d, "r", {"symbols": syms})
