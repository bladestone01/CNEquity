...
from cnequity.diagnostics.report import Severity, build_report
...
def test_missing_package_is_an_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cnequity.diagnostics.report.probe_packages",
        lambda: [
...
def test_relative_data_root_is_an_error(tmp_path):
    report = build_report(config=_config(tmp_path, data_root=Path("./data/cnequity")))
...
def test_unwritable_data_root_is_an_error(tmp_path, monkeypatch):
    ...
    root.mkdir()
    monkeypatch.setattr(
        "cnequity.diagnostics.report._data_root_writable",
        lambda _path: False,
    )
...
def test_doctor_exit_code_follows_report_errors(
    ...
    from cnequity.diagnostics.report import Finding, Report

    monkeypatch.setattr(
        "cnequity.diagnostics.report.build_report",
        lambda config=None, config_path=None: Report(
            environment={"cnequity": "test"},
            packages=[],
