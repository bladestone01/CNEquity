...
# Env: SDE_CONFIG (config path), SDE_LOG_DIR (log destination),
#      SDE_NOTIFY=0 to suppress the desktop notification.
set -uo pipefail
...
if ! health_out="$("$SDE" audit --full --config "$CONFIG" 2>&1)"; then
  problems+=("lake health UNHEALTHY")
fi
echo "$health_out" >>"$LOG"

# 2. Per-dataset freshness gate (any STALE -> exit 1).
if ! status_out="$("$SDE" status --datasets --config "$CONFIG" 2>&1)"; then
...
if [[ ${#problems[@]} -gt 0 ]]; then
  ...
  notify "StockDataEngine 数据异常" "${summary}. ${detail} 见 $LOG"
