...
unit is 手 and its adapter now converts; see ``cnequity.domain.units``.
...
from cnequity.adapters.tdx_protocol._decode import DECODED_ZERO
from cnequity.config import load_config
from cnequity.domain.units import SHARES_PER_LOT
...
def migrate_frame(df: pl.DataFrame) -> tuple[pl.DataFrame, int, int, int]:
    ...
    # cnequity.adapters.tdx_protocol._decode; this clears the rows already
