...
from cnequity.query.views import ensure_duckdb_views  # noqa: E402
...
def measure(con: duckdb.DuckDBPyConnection, start: date, end: date) -> Window:
    ...
    if not names:
        raise SystemExit(
            ...
            "Check `cne status` / coverage_start for daily_bars."
        )
    return Window(
        start=start,
        end=end,
...
# The chart carries prose, so it is localised rather than translated in the
# reader's head: the Chinese README embeds the zh render and the English one the
# en render. Same numbers, same geometry, one string table.
STRINGS = {
    "en": {
        "title": "Survivorship bias in an equal-weight A-share hold",
        "subtitle": "Same basket, same dates — the only difference is whether the "
        "names that delisted are still in it.",
        "complete": "complete universe (delisted names kept)",
        "survivors": "survivors only (what a current roster gives you)",
        "names": "{names} names, {delisted} later delisted",
        "footer": "Delisted names carried to their last printed bar, so the real gap "
        "is wider. Generated by scripts/survivorship_gap.py",
    ...
    "zh": {
        ...
        # PingFang / 微软雅黑 first so the glyphs render on macOS and Windows;
...
def _svg(windows: list[Window], lang: str = "en") -> str:
    t = STRINGS[lang]
    ...
    parts = [
        ...
        f'width="{_W}" height="{_H}" font-family="{t["font"]}">',
        f'<text x="{_PAD_L}" y="26" font-size="16" font-weight="600" fill="{_TEXT}">'
        f"{t['title']}</text>",
        f'<text x="{_PAD_L}" y="46" font-size="12" fill="{_TEXT}">{t["subtitle"]}</text>',
    ]
    ...
    for i, w in enumerate(windows):
        ...
        parts.append(
            ...
            f"{t['names'].format(names=w.names, delisted=w.delisted)}</text>"
        )

    legend_y = _H - 30
    for x, colour, text in (
        (_PAD_L, _COMPLETE, t["complete"]),
        (_PAD_L + t["legend_x"], _SURVIVORS, t["survivors"]),
    ):
        parts.append(
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="en",
        help="Language for the chart's labels. The table stays English either way.",
    )
    ...
    if args.svg:
        ...
        out.write_text(_svg(windows, args.lang), encoding="utf-8")
        print(f"\nWrote {out}")
