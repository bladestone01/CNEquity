...
LABEL="com.stockdataengine.daily"
...
if [[ ! -x "$REPO_ROOT/.venv/bin/sde" ]]; then
  echo "install_scheduler: $REPO_ROOT/.venv/bin/sde not found — create the venv first." >&2
  exit 1
fi

mkdir -p "$DEST_DIR" "$REPO_ROOT/data/stock-data-engine/logs"
...
echo "  logs:     $REPO_ROOT/data/stock-data-engine/logs/"
echo "  verify:   launchctl list | grep stockdataengine"
