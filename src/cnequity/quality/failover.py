...
def fetch_trading_status_backup(
    ...
    spec = failover_spec(config, "trading_status")
    if spec is None or not config.sources.get(spec.backup, True):
        logger.warning("trading_status backup declined: failover not configured")
        return None, {"failover_used": False, "reason": "trading_status failover not configured"}
    if not _baostock_has_day(config, trade_date):
        logger.warning(
            "trading_status backup declined for %s: baostock has not settled the day yet",
    ...
    if sh_sz and bs_df.is_empty():
        ...
        logger.warning("trading_status backup declined: baostock snapshot empty for SH/SZ")
        return None, {
            "failover_used": False,
            "freshness": "fresh",
    ...
    for sym in missing:
        ...
        if is_all_a_symbol(info.code, info.exchange) and not is_cdr_symbol(
    ...
    if fill_failures:
        logger.warning(
            "trading_status backup declined: %d non-tradable missing symbol(s) without fill",
            len(fill_failures),
        )
        return None, {
            "failover_used": False,
    ...
    if n_filled > threshold:
        logger.warning(
            "trading_status backup declined: %d filled rows exceed threshold %d",
            n_filled,
            threshold,
        )
        return None, {
