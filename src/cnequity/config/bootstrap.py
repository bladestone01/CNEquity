...
TEMPLATE_NAME = "cnequity.example.toml"
DEFAULT_USER_CONFIG = Path("configs/cnequity.toml")


def example_toml_text() -> str:
    """Return the packaged example config (works from PyPI wheels and editable installs)."""
    return files("cnequity.config.templates").joinpath(TEMPLATE_NAME).read_text(encoding="utf-8")
...
def render_example_toml(
    *,
    ...
    if plat in ("darwin", "win32"):
        ...
        # workers later after `cne doctor` is green.
...
def write_user_config(
    ...
    *,
    ...
    When *data_root* is omitted, the template's ``./data/cnequity`` is
    resolved to an absolute path so ``cne doctor`` is green on first run
    ...
    root = Path(data_root) if data_root is not None else Path("./data/cnequity")
