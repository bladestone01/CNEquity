...
# Board kline history depth for `asl backfill sector_bars` (~1y+ slack).
...
def _backfill_sector_bars(config: Config, trade_date: date, run_id: str) -> dict:
    ...
    from ashare_lake.adapters.eastmoney.rotation import fetch_sector_bars_history
    ...
    if failed:
        ...
        finding = {
            ...
            "message": (
                ...
                "re-run `asl backfill sector_bars --retry-failed` to resume."
            ),
        }
