...
from ashare_lake.derive.adj_factors import STORED_ADJUST_TYPE
...
from ashare_lake.query.universe import apply_universe_filter
...
def resolve_config(
    *,
    ...
    path = Path("configs/ashare-lake.toml")
    if path.exists():
        return load_config(path)
    raise ReaderError(
        "No config found: pass config= or data_root=, or create configs/ashare-lake.toml"
    )
...
def load(
    ...
    *,
    ...
    config, data_root:
        Lake location; auto-detects ``configs/ashare-lake.toml`` when omitted.
