...
``asl audit`` answers "is the data that landed correct". This answers the other
...
from ashare_lake.domain.datasets import DATASETS, DatasetSpec, is_stale
...
class Gap:
    ...
    def repair_command(self, config_path: str) -> str | None:
        if not self.repairable:
            return None
        cmd = f"asl backfill {self.dataset} --config {config_path}"
...
def _backfillable(spec: DatasetSpec) -> bool:
    """Whether ``asl backfill`` will accept this dataset at all.
