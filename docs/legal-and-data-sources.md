...
优先用发布方而不是转发方（见 [ADR-0006](adr/0006-publishers-over-vendors.md)）：能直接读到编制该数据的机构时，
就不经过转发方。这既降低对单一商业接口的依赖，也降低条款风险——`sources/SOURCES.yml` 里东财与同花顺都标着
`legal_status: restricted`（东财未经书面许可不得再分发，同花顺明确禁止免费产品复制分发），而交易所的公开发布
不带这类限定。
