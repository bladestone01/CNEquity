...
取舍大致围绕下游三件事：回测别被脏数据带偏、日更别悄悄漏、出问题能追到源。更细的分层见 [architecture.md](../architecture.md)。
...
`sde audit --full` 能抓的，别只靠下游自检。例如复权收益极值、adj_factors × corporate_actions 对账、PK 重复、mock 行、分区行数突变。
