...
只读湖面板。`cne serve` 起 uvicorn，提供 `/api/*` JSON 与一个页面。
...
**不打开 `data/duckdb/cnequity.duckdb`。** DuckDB 是「多读或单写」，面板持着读句柄会让夜间跑批里的 `ensure_duckdb_views()` 拿不到写锁——面板会搞挂它在报告的那条流水线。视图在进程私有的内存库里从注册表重建，是毫秒级的。

**不重算审计 findings。** `lake_health()` 要走一遍湖；面板读 `cne audit --full` 已经写好的 `meta/quality/health-latest.json`。一次页面访问不该花一次审计的代价——代价是显示的是上次审计的快照，页面上标了日期。
...
| 端点 | 内容 |
|------|------|
| `GET /api/health` | 锚定交易日、fresh/stale/empty 计数、总行数与体积、findings 分级、历史 all-A 研究门禁、源限制说明、度量表新鲜度 |
...
**行粒度**：`intraday_frequency` 是**行为字段**——它的消费者（audit 的会话检查、reader 的复权集合、`cne backfill --symbols`）都假定存在 `bar_time` 列和「每交易日 N 根」。`trade_ticks` 故意不设它，正是为了不继承这些 bar 形状的代码路径。代价是面板显示 `—`，日内分笔看起来和日频数据集一样。
...
- [CLI 参考 · cne serve](../reference/cli.md#cne-serve)
