...
源码根目录：`src/cnequity/`。包按**数据层与职责**划分，遵循「adapter 薄、step 编排、domain 契约」原则。
...
`import cnequity.steps`（CLI 与 engine 启动时）会执行各 step 模块的 `@register_step`，填充 `STEP_REGISTRY`。新增 step 必须在 `steps/__init__.py` 中 import。
