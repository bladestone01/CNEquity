...
`group="intraday"`，**不在默认 daily wave 上**，且 `[minute_bars].enabled` 默认 false——不该把 8.4GB/年 落在没主动要它的用户头上。入口只有 `cne run daily --group intraday` 和 `cne backfill <dataset>`。
...
`sector_bars` 为 snapshot 语义；历史由 `cne backfill sector_bars` 写入（一次性）。
...
见 `configs/cnequity.example.toml`：
