...
一个免费、零注册、自托管的中国市场金融数据基础设施，从 A 股开始：多源采集，落地 Parquet，保留溯源，并把复权、Universe 和 PIT 口径做成可复查的查询合同。
...
`cne demo` 使用真实 TDX 日线写入独立的 `data/cnequity-demo/`，不会碰全量湖。想直接验证研究口径，再运行：
...
| 论文 / 报告引用 | [引用 cnequity](citation.md) |
...
从 [安装](getting-started/installation.md) 开始；全量页面与模块地图见仓库的[完整文档索引](https://github.com/rootSunc/cnequity/blob/main/docs/README.md)。

项目方向见仓库根目录的 [ROADMAP](https://github.com/rootSunc/cnequity/blob/main/ROADMAP.md)，欢迎通过 [Issues](https://github.com/rootSunc/cnequity/issues) 或 [Discussions](https://github.com/rootSunc/cnequity/discussions) 反馈。
