...
**日更完全可以挂 cron**；贵的是一次性种子，而它已经分片、可续跑（`backfill_chunk_days`，失败会打印 `resume_from`）。

`fetch_workers` 默认留在 1：上面的并发数字是两分钟的突发实测，不是五小时长跑的实测。要开就自己观察一轮。与 `[orchestrator].workers` 无关——日内用线程而非进程池，macOS 上同样可用。
