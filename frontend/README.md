...
Source for the `cne serve` dashboard bundle.

```bash
cd frontend && npm ci && npm run build
```

Outputs are `src/cnequity/serve/static/bundle.js` and
`src/cnequity/serve/static/styles.css`, and both are **committed**. That is
