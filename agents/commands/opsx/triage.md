...
3. **提升动作（promote）**
   ...
   - 原始 note **move** 到 `docs/notes/archive/<原文件名>`，并**在文件顶部加**：
     ...
     ```
   - active 的 INDEX 行**同步移除**（design D3：promoted 移出 active 索引；ADR 另按 AGENTS.md「当前状态」规则登记，速查条目在 `INDEX.md` 登记为 `promoted`）。

4. **折叠动作（fold）**

   - 同 topic 的多条 note 合并为**单一权威产物**（1 个 ADR 或 1 条速查），N 个 INDEX 行压缩为 1 行（速查条目在 `INDEX.md` 登记为 `promoted`，结论 = 速查文件指针）。
