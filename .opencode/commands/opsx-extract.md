...
1. **Select the change to extract from**
   ...
   ```
   Use the **question tool** to let the user select from the list.
...
4. **Present entries for user confirmation**
   ...
   Show each extracted knowledge entry grouped by dimension. Use the **question tool** for each entry with options:
   - "保留并写入知识库" (default/recommended)
   - "跳过此条目"
   - "修改内容后写入"

   For technical constraints, also ask:
   - "写入 docs/technical/CONSTRAINTS.md 作为约束条目" (recommended)
   - "仅写入知识文件作为参考"
...
5. **Write approved entries to knowledge base**
   ...
   | 技术约束 | `docs/technical/CONSTRAINTS.md` | Create new file or append section |
...
- For technical constraints, always offer to write to `docs/technical/CONSTRAINTS.md`
