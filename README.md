...
- **读取时段守卫**：当前交易日在 16:00 之前运行 `cne run daily` 时，`trading_status` 会**跳过采集并提示"非正常数据读取时间段（16:00+ 之后）"**（步骤 `warning` + `check="before_cutoff"` finding），而不是报错——盘中快照本无定稿意义。已补齐当日的 rerun 会直接提示"数据已补齐成功"并短路，不会重复拉取。
